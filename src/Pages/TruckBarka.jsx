...
const TruckBarka = () => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("truckBarka");
    if (stored) {
  ...
  useEffect(() => {
    localStorage.setItem("truckBarka", JSON.stringify(items));
  }, [items]);
  ...
  const handleAdd = () => {
    ...
    const newItem = { name: finalName, quantity: parseInt(quantity), unit, date, updated: false };
